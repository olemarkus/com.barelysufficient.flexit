...
describe('fake-unit manifest (vitest)', () => {
  it('keeps proprietary compatibility objects out of documented supported points', () => {
    const proprietaryAvs = [
      8, 60, 126, 130, 1831, 1833, 1834, 1919, 2090, 2096,
      2113, 2114, 2115, 2118, 2119, 2120, 2121, 2122, 2275,
    ];
    for (const instance of proprietaryAvs) {
      ...
      expect(existsInDocumented, `AV ${instance} should not be in documented points`).toBe(false);
    }
  });
  ...
  it('contains probed AV 1835..1844 present/min/max compatibility values', () => {
    ...
    for (const entry of expected) {
      ...
      expect(maxHint).toMatchObject({
        tag: APPLICATION_TAG.REAL,
        value: entry.max,
      });
      expect(minHint).toMatchObject({
        tag: APPLICATION_TAG.REAL,
        value: entry.min,
      });
    }
  ...
  it('includes AV 1835..1844 as writable documented points', () => {
    for (const instance of [1835, 1836, 1837, 1838, 1839, 1840, 1841, 1842, 1843, 1844]) {
      ...
      expect(point, `Missing AV ${instance}`).not.toBe(undefined);
      expect(point?.access).toBe('RW');
    }
  });
  ...
  it('contains probed proprietary AV compatibility values', () => {
    ...
    expect(av60?.properties[0]).toMatchObject({
      id: PROPERTY_ID.PRESENT_VALUE,
      tag: APPLICATION_TAG.REAL,
      value: 70,
    });
    expect(av1831?.properties[0]).toMatchObject({
      id: PROPERTY_ID.PRESENT_VALUE,
      tag: APPLICATION_TAG.REAL,
      value: 700,
    });
    expect(av1833?.properties[0]).toMatchObject({
      id: PROPERTY_ID.PRESENT_VALUE,
      tag: APPLICATION_TAG.REAL,
      value: 700,
    });
    expect(av1834?.properties[0]).toMatchObject({
      id: PROPERTY_ID.PRESENT_VALUE,
      tag: APPLICATION_TAG.REAL,
      value: 700,
    });
    expect(av2090?.properties[0]).toMatchObject({
      id: PROPERTY_ID.PRESENT_VALUE,
      tag: APPLICATION_TAG.REAL,
      value: 22.999996185302734,
    });
    expect(av2096?.properties[0]).toMatchObject({
      id: PROPERTY_ID.PRESENT_VALUE,
      tag: APPLICATION_TAG.REAL,
      value: 0,
    });
  ...
  it('contains proprietary 5093 overlays for msv42, bv50, and bv445', () => {
    ...
    expect(msv42?.properties[0]).toMatchObject({
      id: FLEXIT_GO_PRIORITY_HINT_PROPERTY_ID,
      tag: APPLICATION_TAG.UNSIGNED_INTEGER,
      value: FLEXIT_GO_PRIORITY_HINT_VALUE,
    });
    expect(bv50?.properties[0]).toMatchObject({
      id: FLEXIT_GO_PRIORITY_HINT_PROPERTY_ID,
      tag: APPLICATION_TAG.UNSIGNED_INTEGER,
      value: FLEXIT_GO_PRIORITY_HINT_VALUE,
    });
    expect(bv445?.properties[0]).toMatchObject({
      id: FLEXIT_GO_PRIORITY_HINT_PROPERTY_ID,
      tag: APPLICATION_TAG.UNSIGNED_INTEGER,
      value: FLEXIT_GO_PRIORITY_HINT_VALUE,
    });
