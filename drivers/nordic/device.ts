...
export = class FlexitNordicDevice extends Homey.Device {
  ...
  private async runLoggedCapabilityOperation<T>(
    ...
    try {
      ...
      const elapsedMs = Date.now() - startedAt;
      const surfacedError = this.buildCapabilityOperationError(unitId, actionDescription, error);
      this.error(
        `Capability '${capability}' ${actionDescription} failed after ${elapsedMs}ms for unit ${unitId};`
        + ` returning error: ${surfacedError.message}`,
        error,
      );
      throw surfacedError;
    } finally {
  ...
  private buildCapabilityOperationError(
    ...
    if (this.isTimeoutError(error)) {
      return new Error(
        `Timed out ${actionDescription} for ${deviceLabel}; the BACnet unit did not respond in time.`,
      );
    }
