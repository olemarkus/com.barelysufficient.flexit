...
import { FlexitCloudClient, CloudToken } from './flexitCloudClient';
...
/**
 * Shared Flexit Go cloud pairing/repair driver. Concrete drivers only declare which
 * product series they own; authentication, model gating, token handling and repair flow
 * live here so the Nordic and EcoNordic cloud drivers never duplicate this logic.
 */
export abstract class FlexitCloudDriver extends Homey.Driver {
  ...
  async onInit() {
    ...
    this.getLogger().info(
      'driver.init',
      `Flexit ${getSeriesLabel(this.series)} cloud driver initialized`,
      { appVersion },
    );
  }
  ...
  async onPair(session: any) {
    ...
    session.setHandler('list_devices', async () => {
      ...
      const supportedPlants = plants.filter(
        (plant) => serialMatchesDriverSeries(plant.serialNumber ?? '', this.series),
      );

      logger.info('driver.pair.devices.listed', 'Listed cloud plants for pairing', {
        plantCount: plants.length,
        matchedCount: supportedPlants.length,
        plants: plants.map((plant) => ({
          plantId: plant.id,
          name: plant.name,
          serialNumber: plant.serialNumber,
          series: classifyFlexitSerial(plant.serialNumber ?? '') ?? 'unknown',
          isOnline: plant.isOnline,
        })),
      });

      return supportedPlants.map((plant) => ({
