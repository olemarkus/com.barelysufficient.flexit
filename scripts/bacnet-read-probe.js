...
function snapshotEntryKey(objectId, property) {
  return `${objectTypeName(objectId?.type)}:${objectId?.instance} ${propertyName(property?.id)}(${property?.id})[${formatIndex(property?.index ?? ASN1_ARRAY_ALL)}]`;
}
...
const NUMERIC_COMPARE_EPSILON = 1e-6;
...
function valuesEqual(left, right) {
  ...
  if (typeof left === 'number' && typeof right === 'number') {
    ...
    return Math.abs(left - right) <= NUMERIC_COMPARE_EPSILON;
  }
...
async function main(argv = process.argv.slice(2)) {
  ...
  const watchPolls = async () => {
    ...
    const sleepInterruptibly = (ms) => new Promise((resolve) => {
    ...
    try {
      while (!stopRequested) {
        ...
        await sleepInterruptibly(options.intervalMs);
      }
    } finally {
      if (handlersAttached) {
